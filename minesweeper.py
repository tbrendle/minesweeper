...
class Game():
    ...
    def drawItem(self, text, pos, color):
        ...
        pygame.draw.rect(
            self.screen, color, (x - width / 2, y - height / 2, width, height))
    ...
    def runMenu(self):
        ...
        tutorial = pygame.transform.scale(
            tutorial, ((x * HEIGHT / 3) / y, HEIGHT / 3))
        x, y = tutorial.get_rect().size
        self.screen.blit(tutorial, (WIDTH / 2 - x / 2, HEIGHT - y))
        self.drawItem(
            'Easy', (WIDTH / 2, HEIGHT - y - 4 * height, width, height), (0, 255, 0))
        self.drawItem(
            'Medium', (WIDTH / 2, HEIGHT - y - 5 * height / 2, width, height), (155, 155, 0))
        self.drawItem(
            'Hard', (WIDTH / 2, HEIGHT - y - height, width, height), (255, 0, 0))
        self.drawItem('Choose a level', (WIDTH / 2, HEIGHT -
                                         y - 11 * height / 2, width, height), (0, 0, 255))
        pygame.display.flip()
        status = KEEP_GOING
    ...
    def initGrid(self):
        ...
        self.display = font.render(
            str(self.grid.emptyValues) + " squares left", 1, (10, 10, 10))
            ...
            for event in ev:
                if hasattr(event, 'key') and event.key == K_ESCAPE:
                    ...
                    if emptyVal != NO_CHANGE:
                        if emptyVal == WIN:
                            self.display = font.render(
                                "You have won, congratz", 1, (10, 10, 10))
                            status = WIN
                        elif emptyVal == GAMEOVER:
                            self.display = font.render(
                                "You have lost, noob !", 1, (10, 10, 10))
                            status = GAMEOVER
                        else:
                            self.display = font.render(
                                str(emptyVal) + " squares left", 1, (10, 10, 10))
                        self.screen.fill((255, 255, 255))
                        self.screen.blit(
                            BACKGROUND, [(WIDTH - XBACK) / 2, (HEIGHT - YBACK) / 2])
                        self.grid.draw(self.screen)
                        self.drawDisplay()
                        pygame.display.flip()
        if status != EXIT:
    ...
    def runPlayAgain(self):
        width = 150
        height = 100
        self.drawItem('It was a nice game ! Do you want to play again ?',
                      (WIDTH / 2, HEIGHT / 2 - 3 * height / 2, 4 * width, height), (0, 0, 255))
        self.drawItem(
            'Yes', (WIDTH / 2 - width, HEIGHT / 2, width, height), (0, 255, 0))
        self.drawItem(
            'No', (WIDTH / 2 + width, HEIGHT / 2, width, height), (255, 0, 0))
        pygame.display.flip()
        status = KEEP_GOING
        while status == KEEP_GOING:
            trigger = pygame.event.wait()
