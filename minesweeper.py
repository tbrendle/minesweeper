...
class Label():
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, self.color, (x-width/2,y-height/2,width,height))
...
class Menu():
    def __init__(self, screen):
        ...
        self.tutorial = pygame.transform.scale(tutorial, ((x*HEIGHT/3)/y, HEIGHT/3))
        x,y = self.tutorial.get_rect().size 
        self.label = Label('Choose a level', (WIDTH/2, HEIGHT-y-11*height/2, width, height), (0,0,255))
        self.buttons = [
                    Button('Easy', (WIDTH/2, HEIGHT-y-4*height, width, height), (0,255,0), 'Easy'),
                    Button('Hard', (WIDTH/2, HEIGHT-y-height, width, height), (255,0,0), 'Hard'),
                    Button('Medium', (WIDTH/2, HEIGHT-y-5*height/2, width, height), (155,155,0), 'Medium'),
                    ]
    def draw(self):
...
class Game():
    def __init__(self, screen, level):
        ...
        width = 150
        height = 100
        self.label = Label(str(self.grid.emptyValues)+" squares left",  (WIDTH/2,25, 500, 50), (255,0,0))
        self.gameOverLabel = Label('It was a nice game ! Do you want to play again ?', (WIDTH/2, HEIGHT/2-3*height/2, 4*width, height), (0,0,255))
        self.buttons = [
            Button('Yes', (WIDTH/2-width, HEIGHT/2, width, height), (0,255,0), True),
            Button('No', (WIDTH/2+width, HEIGHT/2, width, height), (255,0,0), False)
        ]
    def draw(self):
